...
async function simulWar(){
		...
		for(var f of used.fights){
			...
			for(var fighter of fight){fighterUsed.push(fighter.name)}
		}
		...
		for(var side of [0,1]){
			...
			if(!sidiv.length){
				sidiv=div({1:"sidiv"+side,13:300000001,4:[10,side?7:78,"",side?78:7],3:"power",26:1,15:"default",9:uni([{ "font-size":"0.821429rem"
			...
			function bruteClic(brute,side,dv){return function(e){e.preventDefault();if(e.which==2){openBruteCell(brute.name)}else{
				...
				})}else if(fighters[side].length<7){dv.css("background-color","rgba(0,0,0,0.1");fighters[side].push(brute.name)}
				...
				}else{terminateWorkers();res.btn.css("opacity",0.2);res.before.css("opacity",0.2);res.tx.text("...")}
