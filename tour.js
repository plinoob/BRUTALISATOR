...
function endTour(){tourTerminer = true;cl("Tour terminé")}

function isEnded(){return tourEnded}
...
function clickOnProfil(){var elem = $("svg[data-testid^='PersonIcon']")[0];
	if(elem && !clickedOnProfil){clickedOnProfil=true;$(elem).parent().parent().click();setTimeout(function(){$("a[href^='/user']")[0].click();
		setTimeout(function(){var elem=$("div[role^='presentation']")[0]},200);cl(elem,"presentation");elem.click();
	     },500)}}
...
function clickOnHall(){
	...
	tourEnded=true;clickedOnHall=true;
	history.back();return;//dont workvar elem = $("a[href^='/hall']")[0];if(elem && !clickedOnHall){tourEnded=true;clickedOnHall=true;$(elem).click()}
	}
...
async function Tour(){
	while(!tourTerminer){
		...
		else if(url.length==4 && url[2]=="versus"){BRUTE = url[1];currentLoc="Versus";Versus()}
		else if(url.length>2 && url[2] == "fight"){BRUTE = url[1];currentLoc="Fight";Fight()}
		else if(url.length==3 && url[1]=="user"){currentLoc="Profil";Profil()}
		else if(url.length==3 && url[2]=="arena"){BRUTE = url[1];currentLoc="Arena";Arena()}
